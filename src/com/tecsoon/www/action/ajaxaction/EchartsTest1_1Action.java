 ...
 */
@Scope("request")
@Controller("tecsoonecharts-index1_1") // 注释类别名，-struts.xml文件中action的class的名称,添加注释后由spring托管创建
public class EchartsTest1_1Action extends ActionSupport implements ServletRequestAware, SessionAware {
	...
	public String ajaxQuery() {
		...
		try {
//			ChartSettingEntity chartSettingEntity = new ChartSettingEntity();
//			chartSettingEntity.setUserId(String.valueOf(session.get("user")));
...
//			List<Data> dataList = new ArrayList<Data>();
            ...
            if (typeName != null && !"".equals(typeName)) {
				...
				if (session.get("chart1") != null) {
					echartsName = String.valueOf(session.get("chart1"));
					dbName = String.valueOf(session.get("data1"));
				}
			}
			
			// 获取用户配置信息
//            ChartSettingEntity userInfo = chartSettingDao.getChartSettingInfoById(userId);
//			if (userInfo != null) {
//				echartsName = userInfo.getChartType1();
//				dbName = userInfo.getDatasourceType1();
//			}
...
//				Pie pie = new Pie();
				...
				// 标题、半径、位置
				((Pie) mychart).name("radius").radius(80, 400).center("45%", 400);
				((Pie) mychart).roseType(RoseType.radius).itemStyle().normal(normal).emphasis(emphasis);

				((Pie) mychart).data(dataList.toArray());
...
//	    		line3.data(data31, data32, data33, data34, data35, data36, data37);
				...
				// 13_
			} else if ("tecsoonMulti1_1".equals(echartsName)) {
	...
	@SuppressWarnings("unused")
	private List<Data> getDataSourceType2(String dbName) {
		List<DiagramEntity> entLst = null;
		List<ConnectorEntity> connectorList = null;
		...
		if ("datachange1_1".equals(dbName)) {
			...
			for (int i = 0; i < connectorList.size(); i++) {
				ConnectorEntity sbjEntity = connectorList.get(i);
				Data data = new Data();
				data.value(Integer.valueOf(sbjEntity.getNums()));
				dataList.add(data);
			}
		} else if ("datachange1_2".equals(dbName)) {
			entLst = showInfoService.getDiagramCount();
